...
def syr2k_inplace_lower(A_sub, alpha, V, W, beta=1.0):
    C = A_sub.copy(order='F')
    C = blas.dsyr2k(alpha=alpha, a=V, b=W, c=C, lower=1, trans=0, beta=beta, overwrite_c=1)
    _copy_lower_to(C, A_sub)
...
def sytrd_lower(A, nb=32, nx=16):
    ...
    E = np.zeros(n-1, dtype=A.dtype, order='F') if n >= 2 else np.zeros(0, dtype=A.dtype, order='F')
    TAU = np.zeros(n-1, dtype=A.dtype, order='F') if n >= 2 else np.zeros(0, dtype=A.dtype, order='F')
...
if __name__ == "__main__":
    ...
    for n in [2, 4, 8, 16, 32, 64]:
        ...
        A_lapack, D_lapack, E_lapack, tau_lapack, info = dsytrd(A.copy(order='F'), lower=True)
        if info == 0:
